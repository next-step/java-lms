...
- [X] 이미지의 너비가 300px, 높이가 200px을 초과하면 예외가 발생한다.
- [X] 이미지의 너비 높이가 3:2 비율이 아니라면 예외가 발생한다.
- [ ] 무료 강의를 생성한다.
- [ ] 유료 강의를 생성한다.
- [ ] 강의를 생성할 시 시작일이 없으면 예외가 발생한다.
- [ ] 강의를 생성할 시 종료일이 없으면 예외가 발생한다.
- [ ] 강의를 생성할 시 이미지가 없으면 예외가 발생한다.
- [ ] 강의를 생성할 시 무료/유료 여부가 없으면 예외가 발생한다.
- [ ] 강의를 생성할 시료/유료 여부가 없으면 예외가 발생한다.
- [ ] 유료 강의를 생성할 시 최대 수강 인원수가 없으면 예외가 발생한다.
- [ ] 유료 강의를 신청할 시 수강생이 결제한 금액과 수강료가 일치하지 않으면 예외가 발생한다.
- [ ] 강의 신청시 강의가 모집중이 아니라면 예외가 발생한다.