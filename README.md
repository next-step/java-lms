...
### 피드백 하이라이트
- 테스트끼리는 서로 격리되어야 한다! (테스트간에 영향이 없어야한다)
  - 참조글 : https://tecoble.techcourse.co.kr/post/2020-09-15-test-isolation/
  - PK 가 1인지 여부는 중요치 않아요. 하나의 테스트 이후에는 값을 초기화해야한다! 
- 모든 테스트가 개별적으로 격리되어야 하는 이유와 원인 (테스트는 결정적이어야만 한다)
  - 동일한 입력에 대하여 항상 같은 결과를 출력해주지 못하는 테스트를 의미하는데
  - 이유 1 : 테스트코드의 실행순서는 보장되지 않음
  - 이유 2 : 모든 테스트가 (특히 단위테스트) 는 구성요소, 외부 의존성을 최소화하여 테스트하는 이유는 신뢰성 테스트코드가 로직을 검증해줄것이라는 신뢰성, 문제가 발생했을때 재현할수 있도록 하기 위해서 
