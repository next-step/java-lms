...
class SessionTest {
    ...
    public static Session testSession = new Session(LocalDate.now(), LocalDate.now().plusDays(3L), SessionFeeType.FREE, 10);

    @DisplayName("강의 수강신청은 강의 상태가 모집중일 때만 가능하다.")
    @Test
    public void sessionEnrolmentTest_모집중이_아닌_경우() {
        Session session = new Session(LocalDate.now(), LocalDate.now().plusDays(1L), SessionFeeType.FREE, 10);
    ...
    public void sessionEnrolmentTest_종료된_강의를_모집중으로_변경하는_경우() {
        Session session = new Session(LocalDate.now().minusDays(3L), LocalDate.now().minusDays(1L), SessionFeeType.FREE, 10);
    ...
    public void sessionEnrolmentTest_최대수강인원을_초과한_경우() {
        Session session = new Session(LocalDate.now(), LocalDate.now().plusDays(1L), SessionFeeType.FREE, 1);
