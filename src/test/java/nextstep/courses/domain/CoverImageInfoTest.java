...
public class CoverImageInfoTest {
	...
	void create_image_size_exception() {
		int overSize = 1025;
		assertThatThrownBy(() -> new CoverImageInfo(overSize, NORMAL_TYPE, NORMAL_WIDTH, NORMAL_HEIGHT)).isInstanceOf(
			IllegalArgumentException.class).hasMessageContaining(IMAGE_SIZE_OVER_MESSAGE);
	}
	...
	@Test
	@DisplayName("정상적으로 cover image 생성")
	void create_success() {
		assertThatNoException().isThrownBy(
			() -> new CoverImageInfo(NORMAL_SIZE, NORMAL_TYPE, NORMAL_WIDTH, NORMAL_HEIGHT));
	}
