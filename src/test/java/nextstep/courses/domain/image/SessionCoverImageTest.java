...
class SessionCoverImageTest {
  ...
  void 허용된_확장자(String extension){
    assertDoesNotThrow(() -> new SessionCoverImage(SESSION_ID, WIDTH, HEIGHT, extension, VALID_BYTES));
  }

  @ParameterizedTest
  @CsvSource({"webp"})
  void 허용하지않은_확장자는_예외(String extension){
    assertThatThrownBy(() -> new SessionCoverImage(SESSION_ID, WIDTH, HEIGHT, extension, VALID_BYTES))
