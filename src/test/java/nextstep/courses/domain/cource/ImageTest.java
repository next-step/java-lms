...
class ImageTest {
    ...
    public void check_imageSize_and_ratio(){
        Assertions.assertThatThrownBy(() -> Image.of("jpg", 299L, 200L, 1024)).isInstanceOf(ImageSizeBelowMinException.class);

        Assertions.assertThatThrownBy(() -> Image.of("jpg", 299L, 199L, 1024)).isInstanceOf(ImageSizeBelowMinException.class);

        Assertions.assertThatThrownBy(() -> Image.of("jpg", 300L, 201L, 1024)).isInstanceOf(UnsupportedImageRatioException.class);
    }
}
