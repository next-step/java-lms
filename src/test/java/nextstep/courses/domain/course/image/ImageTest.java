...
public class ImageTest {
    ...
    void newObject_over1MBSize_throwsException() {
        assertThatThrownBy(
                () -> new Image(2 * Image.MB, "gif", 300, 200, 1L, LocalDateTime.now())
    ...
    void newObject_lessThanMinWidthSize_throwsException() {
        assertThatThrownBy(
                () -> new Image(Image.MB, "gif", Image.WIDTH_MIN - 1, Image.HEIGHT_MIN, 1L, LocalDateTime.now())
        ).isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void newObject_lessThanMinHeightSize_throwsException() {
        assertThatThrownBy(
                () -> new Image(Image.MB, "gif", Image.WIDTH_MIN, Image.HEIGHT_MIN - 1, 1L, LocalDateTime.now())
        ).isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void newObject_inValidWidthHeightRatio_throwsException() {
        assertThatThrownBy(
                () -> new Image(Image.MB, "gif", 600, 500, 1L, LocalDateTime.now())
        ).isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void newObject_inValidType_throwsException() {
        assertThatThrownBy(
                () -> new Image(Image.MB, "pdf", Image.WIDTH_MIN, Image.HEIGHT_MIN, 1L, LocalDateTime.now())
        ).isInstanceOf(IllegalArgumentException.class);
    }
}
