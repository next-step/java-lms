...
class CourseTest {
  ...
  private static void settingForCourseTest() {
    ...
    Session session1 = new Session(
        ...
        new SessionInfo("자동차 경주", SessionStatus.IN_PROGRESS, true, 0, null),
    ...
    Session session2 = new Session(
        2L,
        new SessionPeriod(LocalDateTime.now(), LocalDateTime.now().plusMonths(1L)),
        new SessionInfo("사다리 타기", SessionStatus.IN_PROGRESS, true, 0, null),
        new CoverImage("사다리 타기 이미지 커버", ImageType.PNG, new CoverImageDetailInfo(1024, 300, 200))
    );

    Session session3 = new Session(
        3L,
        new SessionPeriod(LocalDateTime.now(), LocalDateTime.now().plusMonths(1L)),
        new SessionInfo("수강신청", SessionStatus.IN_PROGRESS, false, 50000, 100),
