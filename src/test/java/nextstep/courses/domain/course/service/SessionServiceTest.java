...
public class SessionServiceTest {
    ...
    void create_success() {
        ...
        when(sessionRepository.findById(savedSession.id())).thenReturn(Optional.of(savedSession));
        ...
        assertThat(findSession.sessionDetail()).isEqualTo(savedSession.sessionDetail());
    }

    @Test
    @DisplayName("수강 신청은 수강 신청 인원에 해당 인원이 추가된다.")
    void apply_success() {
        Session savedSession = SessionFixtures.createdChargedSession(
        ...
        verify(applyRepository).save(ApplyFixtures.apply_one_wait());
    }
    ...
    void changeOnReady_success() {
        ...
        sessionService.changeOnReady(savedSession.id(), SessionFixtures.DATE_2023_12_5);

        Session updatedSession = savedSession.changeOnReady(SessionFixtures.DATE_2023_12_5);
        verify(sessionRepository).update(savedSession.id(), updatedSession);
    }
    ...
    void changeOnGoing_success() {
        ...
        sessionService.changeOnGoing(savedSession.id(), SessionFixtures.DATE_2023_12_6);

        Session updatedSession = savedSession.changeOnGoing(SessionFixtures.DATE_2023_12_6);
        verify(sessionRepository).update(savedSession.id(), updatedSession);
    }
    ...
    void changeOnEnd_success() {
        ...
        sessionService.changeOnEnd(savedSession.id(), SessionFixtures.DATE_2023_12_12);

        Session updatedSession = savedSession.changeOnEnd(SessionFixtures.DATE_2023_12_12);
        verify(sessionRepository).update(savedSession.id(), updatedSession);
    }
    ...
    void approve_success() {
        ...
        assertThat(savedSession.id()).isEqualTo(1L);
        verify(applyRepository).update(ApplyFixtures.apply_one_approved());
    }
    ...
    void cancel_success() {
        ...
        assertThat(savedSession.id()).isEqualTo(1L);
        verify(applyRepository).update(ApplyFixtures.apply_one_canceled());
    }
}
