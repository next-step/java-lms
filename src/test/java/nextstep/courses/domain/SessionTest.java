...
class SessionTest {
    ...
    void registerOpenSession() {
        assertThatCode(() -> new Session(SessionStatus.OPEN).enroll(new Student())).doesNotThrowAnyException();
    }
    ...
    void registerNonOpenSession() {
        ...
        for (SessionStatus status : SessionStatus.values()) {
            ...
            assertThatThrownBy(() -> new Session(status).enroll(new Student()))
                    .isInstanceOf(IllegalArgumentException.class)
    ...
    void enrollAndGetPayment() {
        Session session = new Session(800_000L, 1);
