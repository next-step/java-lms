...
public class SessionTest {
    ...
    void notHave_StartAndEndDate_Test() {
        assertThrows(IllegalArgumentException.class,
                () -> Session.valueOf(1L, "과제4-리팩토링", EnrollmentStatus.RECRUITING, null, null));
    }
    ...
    void haveDateTest() {
        assertDoesNotThrow(()
                -> Session.valueOf(1L, "과제4-리팩토링", EnrollmentStatus.PREPARING
    ...
    void saveSessionImageTest() throws InvalidImageFormatException {
        Session session = Session.valueOf(1L, "과제4 - 레거시 리팩토링", EnrollmentStatus.PREPARING, LocalDate.now(), LocalDate.now());
    ...
    void cannotSignUp_ForPreparingSession_Test() {
        Session session = Session.valueOf(1L, "lms", EnrollmentStatus.PREPARING, LocalDate.now(), LocalDate.now());
    ...
    void cannotSignUp_ForClosedSession_Test() {
        Session session = Session.valueOf(1L, "LMS", EnrollmentStatus.CLOSE, LocalDate.now(), LocalDate.now());
        Payment payment = Payment.freeOf("1", session.getSessionId(), student.getId());
        assertThrows(CannotSignUpException.class, () -> session.signUp(student, payment));
    }
    ...
    void signUp_ForRecruitingSession_Test() throws CannotSignUpException {
        Session session = Session.valueOf(1L,"lms", EnrollmentStatus.RECRUITING, LocalDate.now(), LocalDate.now());
        Payment payment = Payment.freeOf("1", session.getSessionId(), student.getId());
