...
class SessionTest {
    ...
    void 무료강의_신청_모집중_이외에는_예외발생() {
        LocalDateTime now = LocalDateTime.now();
        Session session1 = new Session(1L, "lms", SessionType.FREE, SessionState.READY, "test_02.png", now.plusDays(5), now.plusDays(30));
        assertThatThrownBy(() -> session1.register(NsUserTest.JAVAJIGI))
                .isInstanceOf(CannotRegisterException.class);

        Session session2 = new Session(1L, "lms", SessionType.FREE, SessionState.END, "test_02.png", now.plusDays(5), now.plusDays(30));
    ...
    void 유료강의_신청() throws CannotRegisterException {
        LocalDateTime now = LocalDateTime.now();
        Session session = new Session(1L, "lms", SessionType.PAID, SessionState.RECRUITING, "test_02.png", now.plusDays(5), now.plusDays(30), 10, 5_000);
    ...
    void 유료강의_결제_금액이_다르면_예외발생(long amount) {
        ...
        Session session = new Session(1L, "lms", SessionType.PAID, SessionState.RECRUITING,
                "test_02.png", now.plusDays(5), now.plusDays(30), 2, 5_000);
