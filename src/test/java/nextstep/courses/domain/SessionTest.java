...
class SessionTest {
	...
	public static final Course C2 = new Course("title2", 2L);

	public static final LocalDateTime startDate = LocalDateTime.now();
	public static final LocalDateTime endDate = LocalDateTime.now().plusDays(1);


	@Test
	void 무료강의_최대수강인원제한_성공_테스트() {
		Session Session = new FreeSession(C1, ImageInfoTest.imageInfo, startDate, endDate);
		assertThat(Session.isMaximumNumberOfParticipantsLimited(100)).isTrue();
	}

	@Test
	void 유료강의_최대수강인원제한_성공_실패_테스트() {
		Session Session = new PaidSession(C1, ImageInfoTest.imageInfo, startDate, endDate, 50000, 50);
