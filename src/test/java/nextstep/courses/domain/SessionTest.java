...
class SessionTest {
    ...
    @Test
    void 무료강의생성() {
        Session freeSession = Session.ofFree(LocalDateTime.now(), LocalDateTime.now().plusMonths(1));
    ...
    void 유료강의생성() {
        ...
        Session paidSession = Session.ofPaid(LocalDateTime.now(), LocalDateTime.now().plusMonths(1), sessionPrice, sessionCapacity);
    ...
    void 최대수강인원_exception() {
        int sessionCapacity = 100;
        Session paidSession = Session.ofPaid(LocalDateTime.now(), LocalDateTime.now().plusMonths(1), 1_000_000L, 0);
