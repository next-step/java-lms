...
public class SessionTest {
    ...
    class FreeSessionTest {
        ...
        @Test
        void test03() {
            FreeSession session = new FreeSession(0L, LocalDateTime.now(), LocalDateTime.now().plusDays(1), List.of(SessionCoverImageTest.CI), SessionType.FREE);
            session.changeRecruitmentStatus(RecruitmentStatus.RECRUITING);
            assertThatCode(() -> {
    ...
    class PaidSessionTest {
        ...
        @Test
        void test04() {
            Session session = new PaidSession(0L, 1, 10_000L, LocalDateTime.now(), LocalDateTime.now().plusDays(1), List.of(SessionCoverImageTest.CI), SessionType.PAID);
            session.changeRecruitmentStatus(RecruitmentStatus.RECRUITING);
            assertThatThrownBy(() -> {
        ...
        @Test
        void test05() {
            Session session = new PaidSession(0L, 1, 9_000L, LocalDateTime.now(), LocalDateTime.now().plusDays(1), List.of(SessionCoverImageTest.CI), SessionType.PAID);
            session.changeRecruitmentStatus(RecruitmentStatus.RECRUITING);
