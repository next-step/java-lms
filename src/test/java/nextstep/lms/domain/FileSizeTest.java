...
class FileSizeTest {
    ...
    @Test
    void 너비_확인() {
        assertThatThrownBy(() -> new FileSize(250, 200))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("이미지 너비가 너무 작습니다.");
    }

    @DisplayName("파일 높이가 최소 높이보다 작다면 예외발생")
    @Test
    void 높이_확인() {
        assertThatThrownBy(() -> new FileSize(300, 150))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("이미지 높이가 너무 작습니다.");
    }

    @DisplayName("파일 비율이 지원하지 않는 비율이라면 예외발생")
    @Test
    void 너비_높이_비율_확인() {
        assertThatThrownBy(() -> new FileSize(400, 200))
                .isInstanceOf(IllegalArgumentException.class)
