...
public class QuestionTest {
    ...
    void Question_객체_생성_테스트() {
        ...
        Question question = Question.of(1L, NsUserTest.JAVAJIGI.getUserId(), "title1", "contents1", null);
    ...
    void Question_객체_생성_테스트_아이디가_0() {
        ...
        Throwable exception = Assertions.assertThrows(IllegalArgumentException.class, () -> Question.of(0, NsUserTest.JAVAJIGI.getUserId(), "title1", "contents1", null));
    ...
    void 질문_삭제시_로그인유저의_정보가_유효하지_않은경우() {
        Throwable exception = Assertions.assertThrows(IllegalStateException.class, () -> question.remove(null, answers));
    ...
    void 질문_삭제시_로그인유저의_질문이_아닌_경우() {
        Throwable exception = Assertions.assertThrows(CannotDeleteException.class, () -> question.remove(NsUserTest.SANJIGI, answers));
    ...
    void 질문_삭제시_다른유저의_답글이_달린_경우() {
        ...
        Answer answer = Answer.of(NsUserTest.SANJIGI.getUserId(), question.getId(), "Answers Contents1");
        answers = Answers.of(List.of(answer));

        Throwable exception = Assertions.assertThrows(CannotDeleteException.class, () -> question.remove(NsUserTest.JAVAJIGI, answers));
    ...
    void 질문자와_모든_답변자가_같은_경우() {
        Assertions.assertDoesNotThrow(() -> question.remove(NsUserTest.JAVAJIGI, answers));
    }

    @Test
    @DisplayName("Question 삭제시 답글이 질문을 작성한 사람의 글만 있을 경우 삭제 상태(deleted - boolean type)로 변경되는지 확인 ")
    void 답변이_있는_경우_삭제() {
        question.remove(NsUserTest.JAVAJIGI, answers);
    ...
    void 질문이_이미_삭제된_경우() {
        ...
        question = Question.of(1L, NsUserTest.JAVAJIGI.getUserId(), "title1", "contents1", null, DeleteStatus.DELETED);
    ...
    void 질문자외_답변자들이_존재하는지_확인() {
        Answer answerOne = Answer.of(NsUserTest.SANJIGI.getUserId(), question.getId(), "Answers Contents1");
        Answer answerTwo = Answer.of(NsUserTest.JAVAJIGI.getUserId(), question.getId(), "Answers Contents1");

        answers = Answers.of(List.of(answerOne, answerTwo));
        assertThat(question.hasAnotherOwner(answers)).isTrue();
    }
    ...
    void 삭제_이력_답변이_없는경우() {
        ...
        DeleteHistories deleteHistories = question.remove(NsUserTest.JAVAJIGI, null);
    ...
    void 삭제_이력_답변이_있는경우() {
        ...
        DeleteHistories deleteHistories = question.remove(NsUserTest.JAVAJIGI, answers);
