...
class SessionTest {
  ...
  void enrolment_성공() {
    ...
    this.session.enrollment(this.students, student);

    assertThat(this.students.contains(student)).isTrue();
  }
  ...
  void enrollment_실패_모집중_아님() {
    ...
    assertThatThrownBy(() -> this.session.enrollment(this.students, student))
  ...
  void enrollment_실패_모집인원_초과() {
    ...
    this.session.enrollment(this.students, student1);

    assertThatThrownBy(
        () -> this.session.enrollment(this.students, student2))
        .isInstanceOf(IllegalStateException.class)
  ...
  void enrollment_실패_이미_신청한_사용자() {
    ...
    SessionRegistration sessionRegistration = this.session.getSessionRegistration();
    sessionRegistration.enrolment(this.students, student);
  ...
  void enrollment_실패_수강신청_기간_아님() {
    ...
    assertThatThrownBy(() -> this.session.enrollment(this.students, student1))
        .isInstanceOf(IllegalStateException.class)
        .hasMessage("수강신청 기간이 아닙니다");
    assertThatThrownBy(() -> this.session.enrollment(this.students, student2))
        .isInstanceOf(IllegalStateException.class)
        .hasMessage("수강신청 기간이 아닙니다");
  }
