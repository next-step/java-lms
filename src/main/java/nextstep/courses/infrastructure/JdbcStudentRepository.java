...
public class JdbcStudentRepository implements StudentRepository {
    ...
    public int save(Student student) {
        String sql = "insert into student (session_id) values(?)" ;
        return jdbcTemplate.update(sql, student.getSessionId());
    }

    @Override
    public Student findById(Long studentId) {
        String sql = "select id, session_id from student where id = ?" ;
    ...
    public List<Student> findBySessionId(Long sessionId) {
        String sql = "select id, session_id from student where session_id = ?" ;
