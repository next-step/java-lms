...
public class JdbcSessionRepository implements SessionRepository {
    ...
    public int save(Long courseId, Session session) {
        ...
        final CoverImage coverImage = session.coverImage();
        ...
        if (SessionType.FREE.isSame(sessionType)) {
            ...
            return jdbcTemplate.update(sql, session.title(), sessionPeriod.startDateTime(), sessionPeriod.endDateTime(),
                    sessionStatus.name(), courseId, coverImage.id(), sessionType.name(), recruitStatus.name(), session.createdAt());
        }
        ...
        return jdbcTemplate.update(sql, session.title(), sessionPeriod.startDateTime(), sessionPeriod.endDateTime(),
                ...
                enrollmentCount.maxEnrollmentCount(), enrollmentCount.remainEnrollmentCount(), recruitStatus.name(),
                session.createdAt());
    }
    ...
    public Optional<Session> findById(final Long id) {
        ...
        RowMapper<Session> rowMapper = (rs, rowNum) -> new Session(
                ...
                RecruitStatus.valueOf(rs.getString(12)),
