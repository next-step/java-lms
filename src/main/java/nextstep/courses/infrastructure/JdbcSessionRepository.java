...
public class JdbcSessionRepository implements SessionRepository {
    ...
    public int save(Session session) {
        String sql = "insert into session (course_id, title, cover, cardinal_number, session_cost_type, registration_open_type, state, max_user, start_date, end_date) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        return jdbcTemplate.update(sql, session.getCourseId(), session.getTitle(), session.getCover(), session.getCardinalNumber()
                , session.getSessionCostType().getCode(), session.getRegistrationOpenType().getCode(), session.getSessionRegistration().getState().getCode()
                , session.getSessionRegistration().getMaxUser(), session.getStartDate(), session.getEndDate());
    }

    @Override
    public Optional<Session> findById(long id) {
        String sql = "select id, course_id, title, cover, cardinal_number, session_cost_type, registration_open_type, state, max_user, start_date, end_date from session where id = ?";
