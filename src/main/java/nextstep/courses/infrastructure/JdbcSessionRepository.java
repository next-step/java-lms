...
public class JdbcSessionRepository implements SessionRepository {
    ...
    public Session save(Session session) {
        ...
        final String sqlForSessionSave = "insert into session (title, description, max_number_of_enrollment, fee, status, started_at, ended_at, cover_image_id, course_id, created_at) " +
                "values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        jdbcTemplate.update(connection -> {
            ...
            ps.setTimestamp(6, Timestamp.valueOf(session.getStartedAt()));
            ps.setTimestamp(7, Timestamp.valueOf(session.getEndedAt()));
            ps.setLong(8, session.getIdOfCoverImage());
    ...
    public Session findById(Long id) {
        try {
            final String sqlForSession = "select S.id, S.title, S.description, S.max_number_of_enrollment, S.fee, S.status, S.started_at, S.ended_at" +
            ...
            RowMapper<Session> rowMapperForSession = (rs, rowNum) -> new Session(
                    rs.getLong(1), rs.getString(2), rs.getString(3), new SessionType(rs.getInt(4), rs.getLong(5)), SessionStatus.valueOf(rs.getString(6)),
                    new Period(toLocalDateTime(rs.getTimestamp(7)), toLocalDateTime(rs.getTimestamp(8))),
                    new CoverImage(rs.getLong(9), rs.getString(10), rs.getDouble(11), new Dimensions(rs.getDouble(12), rs.getDouble(13)), ImageType.valueOf(rs.getString(14))),
                    new Course(rs.getLong(15), rs.getString(16), rs.getLong(17), toLocalDateTime(rs.getTimestamp(18)), toLocalDateTime(rs.getTimestamp(19))),
                    toLocalDateTime(rs.getTimestamp(20)), toLocalDateTime(rs.getTimestamp(21)));
    ...
    public Session update(Session session) {
        final String sqlForSessionForUpdate = "update session set title = ?, description = ?, max_number_of_enrollment = ?, fee = ?, status = ?, started_at = ?, ended_at = ?, cover_image_id = ?, updated_at = ? where id = ?";
        jdbcTemplate.update(sqlForSessionForUpdate, session.getTitle(), session.getDescription(), session.getMaxNumberOfEnrollment(), session.getFee(), session.getSessionStatus().name(),
