...
public class Question {
    public static final String NO_AUTHORITY_TO_DELETE = "질문을 삭제할 권한이 없습니다.";
    ...
    public List<DeleteHistory> delete(NsUser loginUser, LocalDateTime now) throws CannotDeleteException {
        if (!isOwner(loginUser)) {
            throw new CannotDeleteException(NO_AUTHORITY_TO_DELETE);
        }

        List<DeleteHistory> deleteHistories = new ArrayList<>();
        deleteHistories.add(new DeleteHistory(ContentType.QUESTION, id, getWriter(), now));
        deleteHistories.addAll(answers.delete(loginUser, now));
        deleted = true;
