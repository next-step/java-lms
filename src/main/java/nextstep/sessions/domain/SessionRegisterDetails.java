...
public class SessionRegisterDetails {
    ...
    public SessionRegisterDetails(long id, Price price, SessionStatus sessionStatus, RecruitmentStatus recruitmentStatus, int maxOfStudents) {
        this(id, price, sessionStatus, recruitmentStatus, maxOfStudents, new Students(maxOfStudents));
    }

    public SessionRegisterDetails(long id, Price price, SessionStatus sessionStatus, RecruitmentStatus recruitmentStatus, int maxOfStudents, Students students) {
        this.id = id;
        this.price = price;
    ...
    public Student enroll(Student student, List<Student> enrolledStudents, Payment payment) {
        if (this.recruitmentStatus.isNotRecruiting()) {
            throw new IllegalArgumentException(String.format("현재 강의는 (%s)인 상태입니다.", this.recruitmentStatus));
        }
