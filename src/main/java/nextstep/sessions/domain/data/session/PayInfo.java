...
public class PayInfo {
    ...
    private void validateSessionFee(PaidType paidType, long fee) {
        if (fee < 0) {
            throw new CannotEstablishSessionException("수강료는 0 이상이어야 합니다.");
        }
        if (paidType.isPaid()) {
    ...
    private void validatePaidSessionFee(long fee) {
        if (fee == 0) {
            throw new CannotEstablishSessionException("유료 강의는 수강료를 입력해야합니다.");
        }
    }

    private void validateFreeSessionFee(long fee) {
        if (fee > 0) {
            throw new CannotEstablishSessionException("무료 강의는 수강료를 입력할 수 없습니다.");
        }
    }

    public void validatePayment(Payment payment) {
        if (!isValidPayment(payment)) {
            throw new CannotEnrollRegistrationException("수강료와 결제한 금액이 다릅니다.");
        }
    }
