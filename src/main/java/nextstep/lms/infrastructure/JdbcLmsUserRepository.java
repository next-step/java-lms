...
public class JdbcLmsUserRepository implements LmsUserRepository {
    ...
    @Override
    public LmsUser findById(Long id) {
        String sql = "select id, user_id, password, name, role, created_at, updated_at from lms_user where id = ?";
        RowMapper<LmsUser> rowMapper = (rs, rowNum) -> new LmsUser(
                rs.getLong(1),
                rs.getString(2),
                rs.getString(3),
                ...
                toLocalDateTime(rs.getTimestamp(6)),
                toLocalDateTime(rs.getTimestamp(7)));
        return Optional.of(jdbcTemplate.queryForObject(sql, rowMapper, id)).orElseGet(null);
    }
